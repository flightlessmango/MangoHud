...
ImGuiCtx::ImGuiCtx(VulkanContext& vk) : vk(vk) {
    ...
    vkImportSemaphoreFdKHR = (PFN_vkImportSemaphoreFdKHR)
        vkGetDeviceProcAddr(vk.device(), "vkImportSemaphoreFdKHR");
    vkGetSemaphoreFdKHR = (PFN_vkGetSemaphoreFdKHR)
        vkGetDeviceProcAddr(vk.device(), "vkGetSemaphoreFdKHR");
};
...
VkSemaphore ImGuiCtx::make_syncfd_semaphore() {
    ...
    if (vkCreateSemaphore(vk.device(), &sci, nullptr, &sem) != VK_SUCCESS) {
...
std::deque<frame> ImGuiCtx::drain_queue() {
    std::deque<frame> local{};
    {
        std::lock_guard<std::mutex> lock(queue_m);
        frame_queue.swap(local);
    }

    for (auto& frame_ : local)
        render(frame_);
...
void ImGuiCtx::render(frame& frame_) {
    ...
    if (ImGui::BeginTable("overlay", cols, ImGuiTableFlags_NoClip)) {
        ...
        for (auto& row : frame_.table.rows) {
            ...
            for (auto& cell : row) {
                ...
                if (!tc.data.empty()) {
                    ...
                    right_aligned(colors.get("FFFFFF"),
                                  ralign_width + ImGui::CalcTextSize("A").x * 2,
                                  "min: %.1fms, max: %.1fms", min, max);
                    ImGui::PopFont();
                    ImGui::TableNextRow();
                    ImGui::TableNextColumn();
                ...
                if (&cell == &row.front())
                    ...
                    right_aligned(tc.vec, ralign_width, "%s", tc.text.c_str());
        ...
        st.window_size = {calculate_width(frame_), calculate_height(frame_)};
        ImGui::PopFont();
        ImGui::EndTable();
        ImGui::PopStyleVar();
        ImGui::End();
...
void ImGuiCtx::submit_draw(frame& frame_) {
    ...
    if (in_flight_release_sem) {
        vkDestroySemaphore(dev, in_flight_release_sem, nullptr);
        in_flight_release_sem = VK_NULL_HANDLE;
    }
    if (in_flight_acquire_sem) {
        vkDestroySemaphore(dev, in_flight_acquire_sem, nullptr);
        in_flight_acquire_sem = VK_NULL_HANDLE;
    }
    ...
    transition_image(cmd, frame_.image,
    ...
    if (frame_.acquire_fd >= 0) {
        acquireSem = make_syncfd_semaphore();
        if (acquireSem == VK_NULL_HANDLE) {
            ::close(frame_.acquire_fd);
            frame_.acquire_fd = -1;
            return;
        }
        ...
        VkImportSemaphoreFdInfoKHR importInfo{
            ...
            .fd = frame_.acquire_fd,
        };

        VkResult ir = vkImportSemaphoreFdKHR(dev, &importInfo);
        if (ir != VK_SUCCESS) {
            ::close(frame_.acquire_fd);
            frame_.acquire_fd = -1;
    ...
    releaseSem = make_syncfd_semaphore();
    if (releaseSem == VK_NULL_HANDLE) {
        if (acquireSem) vkDestroySemaphore(dev, acquireSem, nullptr);
        return;
    }
    ...
    int out_fd = -1;
    VkResult gr = vkGetSemaphoreFdKHR(dev, &getFdInfo, &out_fd);
    if (gr != VK_SUCCESS) {
        out_fd = -1;
    }

    frame_.release_fd = out_fd;

    in_flight_acquire_sem = acquireSem;
