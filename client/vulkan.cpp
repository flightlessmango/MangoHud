...
struct CmdObjects {
  ...
  VkSemaphore in_flight_serverWaitSem = VK_NULL_HANDLE;
  VkSemaphore in_flight_clientDoneSem = VK_NULL_HANDLE;
};
...
static VkSemaphore make_syncfd_semaphore(const vkroots::VkDeviceDispatch* d, VkDevice dev) {
  ...
  if (d->CreateSemaphore(dev, &sci, nullptr, &sem) != VK_SUCCESS)
...
public:
    ...
    VkDevice* pDevice)
  {
    ...
    //   (PFN_vkGetDeviceProcAddr)dispatch->GetInstanceProcAddr(VK_NULL_HANDLE, "vkGetDeviceProcAddr");

    // if (!fpGetDeviceProcAddr) {
    //   fprintf(stderr, "Failed to get vkGetDeviceProcAddr\n");
    //   abort();
    // }

    // ds->vkImportSemaphoreFdKHR =
    //   (PFN_vkImportSemaphoreFdKHR)fpGetDeviceProcAddr(*pDevice, "vkImportSemaphoreFdKHR");
    // ds->vkGetSemaphoreFdKHR =
    //   (PFN_vkGetSemaphoreFdKHR)fpGetDeviceProcAddr(*pDevice, "vkGetSemaphoreFdKHR");

    // if (!ds->vkImportSemaphoreFdKHR || !ds->vkGetSemaphoreFdKHR) {
    //   fprintf(stderr, "Failed to load external semaphore fd fns: import=%p get=%p\n",
    //           (void*)ds->vkImportSemaphoreFdKHR,
    //           (void*)ds->vkGetSemaphoreFdKHR);
    //   // Do not crash later. Either abort or mark feature unsupported.
    //   // abort();
    // }
    std::lock_guard<std::mutex> lock(g_mtx);
...
public:
  ...
  {
    if (!client)
      client = std::make_unique<IPCClient>();
    uint64_t now = os_time_get_nano(); /* ns */
    client->add_to_queue(now);
    ...
    if (!ds->vkImportSemaphoreFdKHR) {
      ds->vkImportSemaphoreFdKHR =
        (PFN_vkImportSemaphoreFdKHR)pDispatch->GetDeviceProcAddr(ds->device, "vkImportSemaphoreFdKHR");
      ds->vkGetSemaphoreFdKHR =
        (PFN_vkGetSemaphoreFdKHR)pDispatch->GetDeviceProcAddr(ds->device, "vkGetSemaphoreFdKHR");
    }
    ...
    if (co->in_flight_serverWaitSem) {
      pDispatch->DestroySemaphore(ds->device, co->in_flight_serverWaitSem, nullptr);
      co->in_flight_serverWaitSem = VK_NULL_HANDLE;
    }
    if (co->in_flight_clientDoneSem) {
      pDispatch->DestroySemaphore(ds->device, co->in_flight_clientDoneSem, nullptr);
      co->in_flight_clientDoneSem = VK_NULL_HANDLE;
    }
    ...
    int server_release_fd = client->take_pending_acquire_fd();
    VkSemaphore serverWaitSem = VK_NULL_HANDLE;

    if (server_release_fd >= 0) {
      serverWaitSem = make_syncfd_semaphore(pDispatch, ds->device);
      if (serverWaitSem != VK_NULL_HANDLE) {
        VkImportSemaphoreFdInfoKHR importInfo{
          .sType = VK_STRUCTURE_TYPE_IMPORT_SEMAPHORE_FD_INFO_KHR,
          .pNext = nullptr,
          .semaphore = serverWaitSem,
          .flags = VK_SEMAPHORE_IMPORT_TEMPORARY_BIT,
          .handleType = VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_SYNC_FD_BIT,
          .fd = server_release_fd,
        };

        VkResult ir = ds->vkImportSemaphoreFdKHR(ds->device, &importInfo);
        if (ir != VK_SUCCESS) {
          ::close(server_release_fd);
          pDispatch->DestroySemaphore(ds->device, serverWaitSem, nullptr);
          serverWaitSem = VK_NULL_HANDLE;
          server_release_fd = -1;
        } else {
          server_release_fd = -1;
        }
    ...
    std::vector<VkPipelineStageFlags> waitStages2;

    waitSems.reserve(pPresentInfo->waitSemaphoreCount + (serverWaitSem ? 1u : 0u));
    waitStages2.reserve(pPresentInfo->waitSemaphoreCount + (serverWaitSem ? 1u : 0u));

    for (uint32_t i = 0; i < pPresentInfo->waitSemaphoreCount; i++) {
      waitSems.push_back(pPresentInfo->pWaitSemaphores[i]);
      waitStages2.push_back(VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
    }

    if (serverWaitSem) {
      waitSems.push_back(serverWaitSem);
      waitStages2.push_back(VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    }

    VkSubmitInfo si{VK_STRUCTURE_TYPE_SUBMIT_INFO};
    si.waitSemaphoreCount = (uint32_t)waitSems.size();
    si.pWaitSemaphores = waitSems.data();
    si.pWaitDstStageMask = waitStages2.data();
    si.commandBufferCount = 1;
    si.pCommandBuffers = &co->cmd;

    VkSemaphore clientDoneSem = make_syncfd_semaphore(pDispatch, ds->device);
    VkSemaphore signalSems[2] = { ds->overlayDone, clientDoneSem };
    si.signalSemaphoreCount = 2;
    si.pSignalSemaphores = signalSems;

    VkResult sr = pDispatch->QueueSubmit(queue, 1, &si, co->fence);
    if (sr != VK_SUCCESS) {
      if (serverWaitSem) pDispatch->DestroySemaphore(ds->device, serverWaitSem, nullptr);
      if (clientDoneSem) pDispatch->DestroySemaphore(ds->device, clientDoneSem, nullptr);
      return pDispatch->QueuePresentKHR(queue, pPresentInfo);
    }

    co->in_flight_serverWaitSem = serverWaitSem;
    co->in_flight_clientDoneSem = clientDoneSem;

    VkSemaphoreGetFdInfoKHR getFdInfo{
      .sType = VK_STRUCTURE_TYPE_SEMAPHORE_GET_FD_INFO_KHR,
      .pNext = nullptr,
      .semaphore = clientDoneSem,
      .handleType = VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_SYNC_FD_BIT,
    };

    int acquire_fd_for_server = -1;
    VkResult gr = ds->vkGetSemaphoreFdKHR(ds->device, &getFdInfo, &acquire_fd_for_server);
    if (gr == VK_SUCCESS && acquire_fd_for_server >= 0) {
      client->send_acquire_fence(acquire_fd_for_server);
      ::close(acquire_fd_for_server);
    }
