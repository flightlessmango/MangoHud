...
hud_table:
  ...
  font_size: 24
  rows:
    - [ {text: GPU, color: "2e9762ff"},
        {value: [GPU, 0, LOAD], unit: "%", precision: 0},
        {value: [GPU, 0, TEMP], unit: "°C", precision: 0} ]

    - [ null,
        {value: [GPU, 0, CORE_CLOCK], unit: "MHz", precision: 0},
        {value: [GPU, 0, POWER], unit: "W", precision: 0} ]

    - [ {text: CPU, color: "2e97cb"},
        {value: [CPU, LOAD], unit: "%", precision: 0},
        {value: [CPU, TEMP], unit: "°C", precision: 0} ]

    - [ null,
        {value: [CPU, FREQ], unit: "MHz", precision: 0},
        {value: [CPU, POWER], unit: "W", precision: 0} ]

    - [ {text: VRAM, color: "ad64c1ff"},
        {value: [GPU, 0, VRAM_USED], unit: "GiB", precision: 1},
        {value: [GPU, 0, VRAM_CLOCK], unit: "MHz", precision: 0} ]

    - [ {text: RAM, color: "c26693"},
        {value: [RAM, USED], unit: "GiB", precision: 1},
        null ]

    - [ {text: VULKAN, color: "eb5b5b"},
        {value: FPS, unit: "FPS", precision: 0},
        {value: FRAMETIME, unit: "ms", precision: 1} ]
...
std::unique_ptr<HudTable> parse_hud_table(const char* file) {
    ...
    try {
        ...
        doc = YAML::Load(std::string{HudYaml});
        parse_yaml(*out, doc);
        return out;
