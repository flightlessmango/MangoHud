...
) : GPU(drm_node, pci_dev, vendor_id, device_id, "gpu-panthor"), FDInfo(drm_node) {
    hwmon.base_dir = hwmon.find_hwmon_dir_by_name("gpu_thermal");
    hwmon.setup(sensors, drm_node);
}

void Panthor::pre_poll_overrides() {
    hwmon.poll_sensors();
    fdinfo.poll_all();
}

int Panthor::get_temperature() {
    return std::round(hwmon.get_sensor_value("temp") / 1000.f);
}

int Panthor::get_core_clock() {
    ...
    std::string freq_str = fds_data[0]["drm-curfreq-panthor"];
...
int Panthor::get_process_load(pid_t pid) {
    uint64_t* previous_gpu_time = &previous_gpu_times[pid];

    uint64_t gpu_time_now = fdinfo.get_gpu_time(pid, "drm-engine-panthor");
...
float Panthor::get_process_vram_used(pid_t pid) {
